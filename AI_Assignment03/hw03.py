...
def logic02_04():
    ...
    ################# Write Your Code Here #########################
    return Forall("$x", Forall("$y",Equiv(Grandmother("$x", "$y"), Exists("$z",AndList([Parent("$x","$z"),Parent("$z","$y"),Female("$y")])))))
...
def number_theorem():
    ...
    formulas.append(Forall("$x",Exists("$y",AndList([Forall("$z", Equiv(Successor("$x","$z"),Equals("$z","$y"))),Successor("$x","$y"), Not(Equals("$x","$y"))]))))
